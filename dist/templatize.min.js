...
// CONCATENATED MODULE: ./src/directives.js
var DIRECTIVES = {
  TO_SYMBOL: {},
  TO_VALUE: {}
};
...
// CONCATENATED MODULE: ./src/nodes.js
...
var nodes_TagNode = /*#__PURE__*/function (_Node3) {
  ...
  function TagNode(raw, inner) {
    ...
    if (!_this3.key.length) throw "Invalid tag: empty evaluation at ".concat(_this3.raw);
...
var nodes_PassToFunctionNode = /*#__PURE__*/function (_Node4) {
  ...
  function PassToFunctionNode(key, contextNode) {
    ...
    _this4.incontext = false; // function can have context directive
    ...
    if (!_this4.key.length) throw "Invalid tag: empty evaluation at ".concat(contextNode.raw);
...
// CONCATENATED MODULE: ./src/misc.js
  ...
  template_createClass(Template, [{
    ...
    value: function parse(delimiters) {
      ...
      while (true) {
        ...
        if (!~close) break; // grab preceding content

        if (open && open > search) {
          current.inner.push(new TextNode(this.template.slice(search, open)));
        }

        search = close + delimiters[1].length; // create node and handle by case

        node = new nodes_TagNode(this.template.slice(open, search), this.template.slice(start, close).trim());
        ...
        switch (node.directive) {
          ...
          case directives.SECTION_END:
            ...
            if (current.open.key !== node.key) throw "Invalid template: Invalid template: section conflict at ".concat(node.raw, " close before inner ").concat(current.start.raw, " closed"); //current.close = node;
      ...
      if (current !== this.root) throw "Invalid template: hanging open section for ".concat(current.start.raw);
    }
  }, {
    key: "indexOf",
    value: function indexOf(search, indexStart) {
      var index = this.template.indexOf(search, indexStart || 0); // ignores escaped
...
// CONCATENATED MODULE: ./src/domain.js
...
var domain_Domain = /*#__PURE__*/function () {
  ...
  domain_createClass(Domain, [{
    ...
    value: function get(key, onFuncError, skipCache) {
      ...
      if (!skipCache && fullkey in this.cache) return this.cache[fullkey]; // can't normal 'get' dynamic children, 'getDynamic'

      if (this.isdynamic) return null; // get context or create if not yet existing
    ...
    key: "getDynamic",
    value: function getDynamic(index) {
      if (!this.isdynamic) return this;
      var dykey = this.fullkey + "~" + index; // get context or create if not yet existing
    ...
    value: function createDynamic(dkey, withData) {
      var dykey = this.fullkey + "~(" + dkey + ")"; // if context exists in children, clear existing first

      if (dykey in this.children) this.children[dykey].cleanup(); // create context
    ...
    value: function _search(fullkey, keysplit, onFuncError, bubble, atstart) {
      ...
      if (!keysplit.length || this.fullkey === fullkey) return this; // to handle names with periods in them (user-error by try to work with), append key parts till match
...
/* harmony default export */ var src_domain = (domain_Domain);
...
var interface_Interface = /*#__PURE__*/function () {
  function Interface(template, options) {
    ...
    this._handleMissingTags = options.handleMissingTags || options.cleanup || interface_DEFAULT.handleMissingTags;
  ...
  interface_createClass(Interface, [{
    ...
    key: "handleMissingTags",
    get: function get() {
      return this._handleMissingTags;
    },
    set: function set(to) {
      if (typeof to !== "undefined") this._handleMissingTags = Boolean(to);
    }
  }, {
    key: "_missingHandler",
    value: function _missingHandler(key, handleMissing) {
    ...
    value: function render(bindings, options) {
      ...
      try {
        ...
        if (bindings instanceof src_domain) {
          // partials get passed context object, don't need to recreate
          this._root = bindings;
        } else {
          this._root = new src_domain(bindings);
        } // map partials
        ...
        for (var pkey in this._partials) {
          if (typeof this._partials[pkey] === "string") {
            try {
              this._partials[pkey] = new src_template(this._partials[pkey]);
            } catch (e) {
              console.error("Invalid partial template for '".concat(pkey, "'"));
              throw e;
            }
          } else if (!(this._partials[pkey] instanceof src_template)) {
    ...
    key: "_context",
    value: function _context(node, domain) {
      var onFuncError = this._errorHandler(node.raw),
          result = {
        node: domain.search(node, onFuncError)
      };

      if (!result.node) return null;

      if (node.func) {
        if (result.node.isdynamic) throw "Cannot pass dynamic data domain to function at ".concat(node.raw);
        ...
        result.value = function () {
          return evalf(result.func, result.node.value(onFuncError), onFuncError);
        };
      } else {
        result.value = function () {
          return result.node.value(onFuncError);
        };
      }
    ...
    key: "_render1",
    value: function _render1(root, domain, processed, isCurrentContext) {
      ...
      for (var n = 0; n < root.inner.length; ++n) {
        ...
        context = isCurrentContext || !node.incontext ? this._context(node, domain) : null; // if null context, it's probably searching into a dynamic domain, so defer till 2nd round
    ...
    key: "_render2",
    value: function _render2(root, domain) {
      domain = domain || this._root;
      var node,
          context,
      ...
      for (var n = 0; n < root.inner.length; ++n) {
        ...
        if (!context) {
          ...
          var dylen = context.node.dynamic.length(),
              dydom = void 0;

          for (var i = 0; i < dylen; ++i) {
    ...
    key: "_section",
    value: function _section(node, domain, context, processed) {
      if (context.node.isdynamic) {
        if (node.inclusive && context.node.dynamic.length()) {
          ...
          this._render1(node, domain, dynode, false); // add copied section node for 2nd round of processing for dynamic data references
      ...
      if (!context.func && Array.isArray(value)) {
        ...
        console.warn("Section evaluation in context passed to function results in array at ".concat(node.raw, ". \n") + "Will be treated as standard section. For repeating sections, must explicitly supply an array value.");
      }
...
;
/* harmony default export */ var src_interface = (interface_Interface);
...
/* harmony default export */ var templatize = __webpack_exports__["default"] = ({
  ...
  from: function from(template, options) {
    return new src_interface(new src_template(template, options), options);
  }
});
