...
const DEFAULT = {
    ...
    escapeAll:          false, 
    handleMissingTags:  "pass"  // other options: throw, error, null
};
...
class Interface {
    ...
    constructor(template, options) {
        ...
        this._handleMissingTags  = options.handleMissingTags || options.cleanup || DEFAULT.handleMissingTags;
    ...
    get handleMissingTags()    { return this._handleMissingTags; }
    ...
    set handleMissingTags(to)  { if(typeof to !== "undefined") this._handleMissingTags = Boolean(to); }

    _missingHandler(key, handleMissing) {
        switch(handleMissing || this.handleMissingTags) {
            case "throw":
            case "error":
    ...
    _section(node, domain, context, processed) {
        ...
        let value   = context.value(), 
            display = node.inclusive === Boolean(value || (this._evalZeroAsTrue && value === 0));
        if(!context.func && Array.isArray(value)) {
            // context passed to function may return array, treat as standard section but warn
            display = node.inclusive === value.length > 0;
            console.warn(
        ...
        if(display) this._render1(node, context.node, processed, true);
    }
