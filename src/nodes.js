...
class TagNode extends Node {
    constructor(raw, inner) {
        ...
        if(!this.inner.length) {
            ...
            // add function node if pass-to-function
            if(this.func) this.func = new PassToFunctionNode(this.func, this);
        }
        // final key check and split
        this.key = this.key.trim();
        if(!this.key.length) throw `Invalid tag: empty evaluation at ${this.raw}`;
        this._finish();
    }
};
...
class PassToFunctionNode extends Node {
    constructor(key, contextNode) {
        super(key);
        this.incontext = false;
        // function can have context directive
        if(this.key[0] === DIRECTIVES.TO_SYMBOL[DIRECTIVES.IN_CONTEXT]) {
            this.key = this.key.slice(1);
            this.incontext = true;
        }
        if(!this.key.length) throw `Invalid tag: empty evaluation at ${contextNode.raw}`;
        this._finish();
    }
};
