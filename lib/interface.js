...
class Interface {
    ...
    _renderOutsideIn(root, domain, processed, unresolved) {
        ...
        for(let n = 0; n < root.inner.length; ++n) {
            ...
            if(domain.isrepeating && (node.incontext || node.func.find(fn => fn.incontext))) {
                processed.inner.push(node);
                continue;
            }
            ...
            if(unresolved.length && checkNode || node.func.length) {
                ...
                for(let u = 0; u < unresolved.length; ++u) {
                    ...
                    if(node.func.find(fn => unresolved[u].incontext(fn.key))) {
                        cantResolve = true;
                        break;
                    }
                }
    ...
    _section(node, context, processed, unresolved) {
        ...
        // Standard sections simply recurse inner content to render. Only thing is checking for creation of 
        // dynamic data context first.
        let domain = context.toDyanmicDomain();
