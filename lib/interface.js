...
class Interface {
    ...
    render(bindings, options) {
        ...
        if(options.errorOnMissingTags !== undefined) this.errorOnMissingTags = options.errorOnMissingTags;
        ...
        try {
            ...
            this._options = {
                ...
                errorOnMissingTags: this.errorOnMissingTags
            };
    ...
    _partial(node, context) {
        if(!this._partials[node.key]) {
            if(this.error_on_missing_tags) throw `Render error: missing partial for ${node.key}`;
