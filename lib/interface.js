...
class Interface {
    ...
    _renderOutsideIn(root, domain, processed, unresolved) {
        ...
        for(let n = 0; n < root.inner.length; ++n) {
            ...
            // render sections
            if(forSection) {
                // // copy empty section node to preserve context hierarchy -- excluding section-passed-as 
                // // context as they don't function as new level in hierarchy
                // if(!node.passcontext) {
                //     processSection = new SectionNode(node);
                //     processed.inner.push(processSection);
                // } else {
                //     processSection = processed;
                // }
                // handler split out for readability, but from there can recurse back into this function
