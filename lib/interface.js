...
class Interface {
    ...
    _context(node, domain) {
        ...
        if(node.func) {
            if(result.node.isdynamic) return null; //throw `Cannot pass dynamic data domain to function at ${node.raw}`;
    ...
    _render1(root, domain, processed, isCurrentContext) {
        ...
        for(let n = 0; n < root.inner.length; ++n) {
            ...
            // if null context, it's probably searching in a dynamic data domain, so defer till 2nd round
            if(!context) {
                processed.inner.push(node);
                continue;
            }
