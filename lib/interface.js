...
function _importPartials(given) {
    if(!given) return {};
    var imported = {};
    for(let pkey in given) {
        if(typeof given[pkey] === "string") {
            try {
                imported[pkey] = new Template(given[pkey]);
            } catch(e) {
...
class Interface {
    ...
    constructor(template, options) {
        ...
        this._root               = null;
        this._ogPartials         = _importPartials(options.partials);
        this._partials           = {};
        this._options            = {};
    ...
    render(bindings, options) {
        options = options || {};
        let renderOptions = {
            errorOnFuncFailure: !options || options.errorOnFuncFailure === undefined ? this.errorOnFuncFailure : options.errorOnFuncFailure, 
            evalZeroAsTrue:     !options || options.evalZeroAsTrue     === undefined ? this.evalZeroAsTrue     : options.evalZeroAsTrue, 
            escapeAll:          !options || options.escapeAll          === undefined ? this.escapeAll          : options.escapeAll, 
            errorOnMissingTags: !options || options.errorOnMissingTags === undefined ? this.errorOnMissingTags : options.errorOnMissingTags
        };
        ...
        try {
            ...
            let addPartials = _importPartials(options.partials)
    ...
    _processContext(node, domain, dynamics) {
        ...
        if(node.func) {
            ...
            if(node instanceof SectionNode && node.ascontext) {
    ...
    _section(node, context, processed, unresolved) {
        ...
        if(context.isrepeating || node.ascontext) {
            if(node.inclusive && (node.ascontext || context.length)) {
