...
class Interface {
    ...
    _processContext(node, domain) {
        ...
        // evaluate properties of value context
        ...
        if(!node.func) {
            ...
            // reevaluate properties of value context
        ...
        for(let n = 0; n < root.inner.length; ++n) {
            ...
            if(!context.isrepeating) {
                if(this._display(node.inclusive, useDomain)) {
                    processed.inner.push(this._renderInsideOut(node, useDomain));
                }
                continue;
            }
            ...
            for(let i = 0; i < context.length; ++i) {
                ...
                if(this._display(true, dydom)) {
                    processed.inner.push(this._renderInsideOut(node, dydom));
                }
            }
        }
    ...
    _display(inclusive, domain) {
        ...
        switch(domain.type) {
            case TYPES.OBJECT:
                ...
                break;
            case TYPES.ARRAY:
                // shouldn't happen, should be treated as repeating section first
                display = inclusive && display.length;
                console.warn("Array passed to _display");
                break;
            default:
