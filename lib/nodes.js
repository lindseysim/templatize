        ...
        if(!this.inner.length) {
            ...
            if(!this.directive) {
                ...
                switch(this.directive) {
                    ...
                    case DIRECTIVES.FORMAT:        // not possible as length=2 but just to be thorough
            ...
            if(split.length === 2) {
                if(!split[0]) { split = [split[1]]; } else if(!split[1]) { split = [split[0]]; }
            }
            if(split.length > 1) {
                if(split.length > 2) {
...
class PassToFunctionNode extends Node {
    constructor(key, contextNode) {
        super(key);
        this.incontext = false;
        // function can have context directive, but can't be pure context -- e.g. {{context~.}} no make sense
        if(this.key[0] === DIRECTIVES.TO_SYMBOL[DIRECTIVES.IN_CONTEXT]) {
            this.key = this.key.slice(1);
            this.incontext = true;
        }
        if(!this.key.length) {
            if(this.incontext) throw `Invalid tag: function in-context must still have relative path ${contextNode.raw}`;
            throw `Invalid tag: empty evaluation at ${contextNode.raw}`;
        }
        this._finish();
    }
};
...
class SectionNode extends Node {
    constructor(tag, parent) {
        ...
        if(tag instanceof SectionNode) {
            ...
            this.inclusive = Boolean(tag.directive === DIRECTIVES.SECTION_INC);
            this.open      = tag;
            this.close     = null;
