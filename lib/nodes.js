...
function isCombinedDirective(firstDirective, secondDirective) {
    if(firstDirective && this.directive !== firstDirective) return false;
    if(!secondDirective) return true;
    let start = !firstDirective ? 0 : DIRECTIVES.TO_SYMBOL[firstDirective].length, 
        offset = start + DIRECTIVES.TO_SYMBOL[secondDirective].length;
    return DIRECTIVES.TO_VALUE[this.inner.slice(start, offset)] === secondDirective;
}
...
class TagNode extends Node {
    constructor(raw, inner) {
        ...
        if(!this.inner.length) {
            ...
            if(!this.directive) {
                this.directive = 0;
            } else if(isCombinedDirective.call(this, DIRECTIVES.LIST, DIRECTIVES.SECTION_INC)) {
            ...
            } else if(isCombinedDirective.call(this, DIRECTIVES.SECTION_INC, DIRECTIVES.PASS_CONTEXT)) {
                // special case of section pass as context
                this.directive = DIRECTIVES.PASS_SECTION;
                this.func =this.key = this.key.slice(DIRECTIVES.TO_SYMBOL[this.directive].length);
            } else {
...
class SectionNode extends Node {
    constructor(tag, parent) {
        ...
        if(tag instanceof SectionNode) {
            ...
            this.ascontext = this.inclusive && !this.list && tag.directive === DIRECTIVES.PASS_SECTION;
            this.inclusive = this.inclusive || tag.directive === DIRECTIVES.SECTION_INC;
            ...
            if(!this.inclusive && tag.directive !== DIRECTIVES.SECTION_EXC) {
