...
function parseFormatDirectives() {
    let hasfuncs = this.func && this.func.length, 
        sym = DIRECTIVES.TO_SYMBOL[DIRECTIVES.FORMAT], 
        split = (hasfuncs ? this.func[this.func.length-1] : this.key).split(sym);
    ...
    if(split.length > 1) {
        ...
        this.format = split[1];
        if(hasfuncs) {
            // remove format directive from last function, apply to tag as whole
            this.func[this.func.length-1] = split[0];
        } else {
            this.key = split[0];
        }
    }
    ...
    split = hasfuncs ? this.func[this.func.length-1] : this.key;
    if(split.endsWith(sym)) {
        this.escape = true;
        split = split.slice(0,-sym.length);
        if(hasfuncs) {
            // remove format directive from last function, apply to tag as whole
            this.func[this.func.length-1] = split;
        } else {
            this.key = split;
        }
    }
