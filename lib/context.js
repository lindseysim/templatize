...
class Context {
    ...
    constructor(node, domain, dynamics, onFuncError) {
        ...
        if(this.node.func.length) {
            ...
            if(~this.func.findIndex(fn => !fn)) throw `Context passed to unresolved function at ${this.node.raw}`;
            if(~this.func.findIndex(fn => !fn.function)) throw `Context passed to non-function at ${this.node.raw}`;
        }
    }
