...
var nodes_PassToFunctionNode = /*#__PURE__*/function (_Node4) {
  ...
  function PassToFunctionNode(key, contextNode) {
    ...
    _this4.incontext = false; // function can have context directive, but can't be pure context -- e.g. {{context~.}} no make sense
    ...
    if (!_this4.key.length) {
      if (_this4.incontext) throw "Invalid tag: function in-context must still have relative path ".concat(contextNode.raw);
      throw "Invalid tag: empty evaluation at ".concat(contextNode.raw);
    }
  ...
  interface_createClass(Interface, [{
    ...
    value: function _render1(root, domain, processed, isCurrentContext) {
      ...
      for (var n = 0; n < root.inner.length; ++n) {
        ...
        context = isCurrentContext || !node.incontext ? this._context(node, domain) : null; // if null context, it's probably searching in a dynamic data domain, so defer till 2nd round
