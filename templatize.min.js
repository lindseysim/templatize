...
var TagNode = /*#__PURE__*/function (_Node3) {
  ...
  function TagNode(raw, inner) {
    ...
    if (!_this3.tagstr.length) {
      ...
      split = (_this3.func.length ? _this3.func[_this3.func.length - 1] : _this3.key).split(sym);
      ...
      split = _this3.func.length ? _this3.func[_this3.func.length - 1] : _this3.key;
      if (split.endsWith(sym)) {
        _this3.escape = true;
        split = split.slice(0, -sym.length);
        if (_this3.func.length) {
          // remove format directive from last function, apply to tag as whole
          _this3.func[_this3.func.length - 1] = split;
        } else {
          _this3.key = split;
        }
      }
  ...
  context_createClass(Context, [{
    ...
    value: function resolve(root) {
      ...
      if (this.node instanceof SectionNode && this.node.passcontext) {
        this.value = function (context) {
          var iterval = context || _this2.domain.value(_this2._onError);
          _this2.func.forEach(function (fn) {
            iterval = evalf(fn["function"], iterval, rootDomain, _this2._onError);
          });
          return iterval;
        };
        this._onError = null; // reference no longer needed
      ...
      this.value = this.domain.value(this._onError);
      this.func.forEach(function (fn) {
        _this2.value = evalf(fn["function"], _this2.value, rootDomain, _this2._onError);
      });
      this._onError = null; // reference no longer needed
      this.isresolved = true;
    ...
    value: function toDynamicDomain() {
      ...
      var value = this.value,
        fnchain = "";
      this.func.forEach(function (fn, i) {
        value = fn(value);
        fnchain += "".concat(i && "->" || "").concat(fn.fullkey, "()");
      });
  ...
  interface_createClass(Interface, [{
    ...
    value: function _renderOutsideIn(root, domain, processed, unresolved) {
      ...
      for (var n = 0; n < root.inner.length; ++n) {
        ...
        if (domain.isrepeating && (node.incontext || node.func.find(function (fn) {
          return fn.incontext;
        }))) {
          processed.inner.push(node);
          continue;
        ...
        if (unresolved.length && checkNode || node.func.length) {
          ...
          var _loop = function _loop(u) {
            ...
            if (node.func.find(function (fn) {
              return unresolved[u].incontext(fn.key);
            })) {
              cantResolve = true;
              return "break";
            }
    ...
    value: function _renderInsideOut(root, domain, dynamics) {
      ...
      for (var n = 0; n < root.inner.length; ++n) {
        ...
        if (node.passcontext && !context.isresolved) {
          var rendered = this._renderInsideOut(node, useDomain, dynamics);
    ...
    value: function _section(node, context, processed, unresolved) {
      ...
      var domain = context.toDyanmicDomain();
    ...
    value: function _renderValue(node, value) {
      ...
      if (node.directive === directives.LIST && type === TYPES.ARRAY) {
        value = value.map(function (vi) {
          ...
          return formatValue(vi, format, node.escape || _this2._escapeAll);
        });
        switch (value.length) {
          case 0:
            return "";
