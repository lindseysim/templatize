...
var domain_Domain = /*#__PURE__*/function () {
  ...
  domain_createClass(Domain, [{
    ...
    key: "getDynamic",
    value: function getDynamic(index, context) {
      if (!this.isdynamic) return this;
      var dykey = "".concat(this.fullkey, "<~>").concat(index); // get context or create if not yet existing
    ...
    value: function createDynamic(dkey, withData) {
      var dykey = "".concat(this.fullkey, "<~>(").concat(dkey, ")"); // if context exists in children, clear existing first
...
var interface_Interface = /*#__PURE__*/function () {
  ...
  interface_createClass(Interface, [{
    ...
    key: "_context",
    value: function _context(node, domain, lastDomain) {
      // in-context directive in unresolved, dynamic domchain can't be resolved yet
      if (lastDomain && node.incontext && lastDomain.isdynamic) return null; // get domain of node
      ...
      if (!result.node) return null; // can't resolve if resulting node's domain is dynamic (exception for lists and section evaluation)

      if (result.node.isdynamic && !(node instanceof nodes_SectionNode) && node.directive !== directives.LIST) {
        return null;
      }
      ...
      if (node.func) {
        ...
        result.value = function () {
          return evalf(result.func["function"], result.node.value(onFuncError), onFuncError);
        };
      } else {
        result.value = function () {
    ...
    key: "_render1",
    value: function _render1(root, domain, processed, lastDomain) {
      ...
      for (var n = 0; n < root.inner.length; ++n) {
        ...
        context = this._context(node, domain, lastDomain); // if null, possibly due to nesting into dynamic data context, so defer till 2nd round
    ...
    value: function _render2(root, domain) {
      ...
      for (var n = 0; n < root.inner.length; ++n) {
        ...
        if (!context) {
          ...
          var dylen = context.dynamic.length(),
              dydom = void 0;
    ...
    value: function _section(node, domain, context, processed) {
      if (context.node.isdynamic) {
        if (node.inclusive && context.node.dynamic.length()) {
          ...
          var dynode = new nodes_SectionNode(node); // replace any context shortcuts with full path as it will be handled later out of context
          ...
          this._render1(node, domain, dynode, context.node); // add copied section node for 2nd round of processing for dynamic data references
